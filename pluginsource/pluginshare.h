...
typedef __int64 TNoeSize;
#else
typedef int TResvInt;
typedef int TNoeSize;
#endif
...
#define NOESIS_PLUGINAPI_VERSION	83 //make your plugin require this version if you use new api functions.
//Noesis 4.465 - 83
//Noesis 4.461 - 82
//Noesis 4.46 - 81
//Noesis 4.457 - 80
...
#ifdef _NOE64
typedef unsigned __int64 noesisIntPtr_t;
#else
...
typedef struct noesisAnim_s noesisAnim_t;
typedef struct mdlMemLocal_s mdlMemLocal_t;
typedef struct noeSharedGL_s noeSharedGL_t;
typedef struct noesisSplineSet_s noesisSplineSet_t;
...
typedef struct mathImpFn_s
{
	...
	//for giving these a lerp suffix, you have my deepest apologies
	...
	unsigned __int64 (*Math_TransformBits)(const unsigned __int64 val, const unsigned int bitCount, const unsigned char *pBitMatrix);
	unsigned __int64 (*Math_TransformBitsReverse)(const unsigned __int64 val, const unsigned int bitCount, const unsigned char *pBitMatrix);
	unsigned __int64 (*Math_TransformBitsInverse)(const unsigned __int64 val, const unsigned int bitCount, const unsigned char *pBitMatrix);
	unsigned __int64 (*Math_TransformBitsInverseReverse)(const unsigned __int64 val, const unsigned int bitCount, const unsigned char *pBitMatrix);
...
struct SNoeDebugDataParams
{
	//i can no longer remember why i'm not including stdint.h and can't be bothered to try building the older plugins
	//so i'm continuing to do stupid things, hell yes
	unsigned __int64 mAddr;
	unsigned __int64 mResv[4];
	void *mpUserData;
};
...
typedef struct noePluginFn_s
{
	...
	//returns true if write was successful
	bool		(*NPAPI_SaveImageFromTexture)(wchar_t *filename, noesisTex_t *tex, char *options);

	//returns true if successful (will always fail outside of gui mode)
	bool		(*NPAPI_SetSelectedDirectory)(const wchar_t *pPath);

	//pushes/pops current RAPI context, safely preserving module and global memory status. pResv must be NULL.
	//always take care to call pop after push, failure to do so results in undefined behavior. (probably crashes)
	//push will invalidate the current module, meaning an explicit module state must be set by the plugin via NPAPI_SetGlobalMemForRAPI before any rapi calls are valid.
	void (*NPAPI_PushRAPIContext)(void *pResv);
	void (*NPAPI_PopRAPIContext)(void *pResv);

	int (*NPAPI_JaguarRISCDisassemble)(char *pOut, const unsigned char *pData, const int dataSize, const int maxOpCount, const bool isGPU);

	bool (*NPAPI_DesktopCapStart)(const int capMode, const int capDelay, const int capQuality, const int capShift);
	void (*NPAPI_DesktopCapEnd)();
	const unsigned char *(*NPAPI_DesktopCapGetOldestBuffer)(unsigned int *pSizeOut, unsigned int *pWidthOut, unsigned int *pHeightOut);
	void (*NPAPI_DesktopCapClearOldestBuffer)();

	//debugger functions return < 0 on failure, return value is otherwise subjective
	int			(*NPAPI_RegisterDebuggerDataHandler)(const char *pTitle, int (*pCallback)(const SNoeDebugDataParams *pParams), void *pUserData);
	int			(*NPAPI_RegisterDebuggerTickCallback)(int (*pCallback)(void *pUserData), void *pUserData);
	//fairly rigid (only synchronous mode is offered at the moment), not bothering to implement access to additional debugger-queried state at the moment since i don't expect anyone else
	//to ever use any debugger-related functionality.
	int			(*NPAPI_DebuggerReadData)(void *pDst, const unsigned __int64 readAddr, const unsigned __int64 readSize, const noesisIntPtr_t resv);
	int			(*NPAPI_DebuggerWriteData)(const void *pSrc, const unsigned __int64 writeAddr, const unsigned __int64 writeSize, const noesisIntPtr_t resv);

	int (*NPAPI_DesktopCapAvailableBufferCount)();
...
#define NMATFLAG2_UV1_ANY				(NMATFLAG2_OCCL_UV1 | NMATFLAG2_OPACITY_UV1)
#define NMATFLAG2_UV2_ANY				(NMATFLAG2_OPACITY_UV2)
...
#define NMATFLAG2_PBR_METAL_SPECBLUE	(1<<14) //spec color in blue for metallic path (note that this is subject to the specular swizzle)
#define NMATFLAG2_BLEND_MAX				(1<<15) //use max blend equation (if available in renderer/hardware)
...
struct SNoeTexExtraData
{
	float			mLodBias;

	//don't touch anything below here
	int				mInternalFlags;
	int				mInternalTracking[8];
	TResvInt		mResv[8];
};
typedef struct noesisTex_s
{
	...
	SNoeHDRTexData	*pHdr;
	SNoePalData		*pPal;
	SNoeTexExtraData *pExtra;
	TResvInt		resv[2]; //THESE VALUES MUST BE 0 (this is done by Noesis_TextureAlloc)
} noesisTex_t;
...
#define RPGOPT_FIXTRIWINDINGS			(1<<13) //make triangle windings match vertex normals (usually a terrible hack, but useful for games which just disabled face culling)
#define RPGOPT_NOINDEXLIMIT				(1<<14) //disables index limiting, use at your own risk
...
typedef enum
{
	...
	NOEKF_ROTATION_EULER_SINGLE,
	NOEKF_ROTATION_MATRIX_33,
...
typedef enum
{
	...
	NOEKF_SCALE_TRANSPOSED_VECTOR_3, //deprecated, use one of the other types with KFBONEFLAG_SCALE_TP instead
...
typedef enum
{
	...
	NOEKF_INTERPOLATE_CATMULLROM,
	NOEKF_INTERPOLATE_TCB, //expects 3 extra floats (tension, continuity, bias) on the end of each key
	NOEKF_INTERPOLATE_HERMITE, //hermite with explicit tangents, expects Noesis_KF_DataCountFor*Type(...) * 2 extra floats (in tangent followed by out tangent)
	NOEKF_INTERPOLATE_BEZIER, //bezier with explicit tangents, same requirements as NOEKF_INTERPOLATE_HERMITE
	NOEKF_INTERPOLATE_BEZIER_R, //as above, with local tangents
...
#define KFDFLAG_EASE				(1 << 3) //expects 2 floats following the data type's floats (as well as any additional data based on interpolation type) - ease in and ease out
#define KFDFLAG_WRAP_TO_LAST		(1 << 4) //wrap from first around to last key when interpolating
#define KFDFLAG_WRAP_TO_FIRST		(1 << 5) //wrap from last around to first key when interpolating
#define KFDFLAG_SMOOTHTANGENTS		(1 << 6) //smooth in/out tangents on curve when applicable
#define KFDFLAG_EULER_SINGLE_AXIS	(1 << 7) //if set, uses bits 9 and 10 as a rotation axis index (only applicable to NOEKF_ROTATION_EULER_SINGLE)
#define KFDFLAG_SET_SINGLE_AXIS(flags, axisIndex) flags |= ((axisIndex << 8) | KFDFLAG_EULER_SINGLE_AXIS)
#define KFDFLAG_GET_SINGLE_AXIS(flags) ((flags & 0x300) >> 8)
...
#define KFBONEFLAG_NLERP			(1<<3) //normalize rotation after interpolation
#define KFBONEFLAG_NLERP_TP			(1<<4) //as above, but normalize transposed matrix
#define KFBONEFLAG_SCALE_TP			(1<<5) //all scale transforms are transposed
...
#define TEXRGBAFLOAT_FLAG_NOHDR					(1 << 0) //don't use hdr data when available
#define TEXRGBAFLOAT_FLAG_SCALEANDBIAS			(1 << 1) //scale & bias from 0..1 to -1..1
#define TEXRGBAFLOAT_FLAG_RGB					(1 << 2) //return rgb array instead of rgba
#define TEXRGBAFLOAT_FLAG_TOLINEAR				(1 << 3) //convert to linear space (excludes alpha)
#define TEXRGBAFLOAT_FLAG_TOGAMMA				(1 << 4) //convert to gamma space (excludes alpha)
#define TEXRGBAFLOAT_FLAG_NORMALIZE				(1 << 5) //normalize each pixel (either xyz or xyzw depending on presence of TEXRGBAFLOAT_FLAG_RGB)
...
typedef struct noeRAPI_s
{
	...
	bool				(*Noesis_WriteFile)(const char *filename, void *data, TNoeSize dataSize);
	bool				(*Noesis_WriteFileMakePath)(const char *filename, void *data, TNoeSize dataSize);
	...
	noesisAnim_t		*(*Noesis_AnimAlloc)(char *filename, BYTE *data, TNoeSize dataLen);
	...
	void							(*Noesis_SetExtraAnimData)(BYTE *animData, TNoeSize animDataSize);
	BYTE							*(*Noesis_GetExtraAnimData)(TNoeSize &animDataSize);
	...
	bool				(*Noesis_WriteAnimFile)(char *animName, char *extension, BYTE *buf, TNoeSize len);
	...
	bool				(*Noesis_WriteFileW)(const wchar_t *filename, void *data, TNoeSize dataSize);
	bool				(*Noesis_WriteFileMakePathW)(const wchar_t *filename, void *data, TNoeSize dataSize);
	...
	//New in Noesis 4.079
	noesisTex_t			*(*Noesis_TextureAllocEx)(char *filename, int w, int h, BYTE *data, TNoeSize dataLen, int type, int flags, int mipCount);
	...
	BYTE				*(*Noesis_ConvertDXTEx)(int w, int h, BYTE *data, DWORD dxtFmt, TNoeSize dataSize, convertDxtExParams_t *params, TResvInt resvB);
	...
	SNoeHDRTexData		*(*Noesis_AllocHDRTexStructure)(void *pData, const TNoeSize dataLen, const ENoeHdrTexFormat hdrTexFormat, void *pResvA, void *pResvB);
	...
	BYTE				*(*Image_GetTexRGBAOffset)(noesisTex_t *tex, bool &shouldFree, TNoeSize offset);
	float				*(*Image_GetTexRGBAFloatOffset)(noesisTex_t *tex, bool &shouldFree, TNoeSize offset, bool useHdr);

	TNoeSize			(*Image_GetMipSize)(const noesisTex_t *tex, const int mipIndex, const bool useHdr);
	...
	SNoePalData			*(*Noesis_AllocPalTexStructure)(void *pData, const TNoeSize dataLen, const int colorCount, const ENoePalFormat palTexFormat, void *pResvA, void *pResvB);
	...
	float				*(*Image_GetTexRGBAFloatWithFlags)(noesisTex_t *tex, bool &shouldFree, bool &outputIsHDR, TNoeSize offset, unsigned int flags);

	//like the first version, but flags allows for different sample distributions
	BYTE				*(*Image_NormalMapFromHeightMapEx)(const BYTE *heightRGBA, const int heightW, const int heightH, const float heightScale, const float texelScale, const int flags);

	//pResv should be NULL. this will be pool-allocated, no need to explicitly free or worry about attaching to a bone.
	SBoneDebugInfo		*(*Noesis_AllocBoneDebugInfo)(void *pResv);

	//remap fraction with 3ds-style ease in/out
	float				(*Noesis_EaseInterpolationFraction)(const float frac, const float easeIn, const float easeOut);

	double				(*Noesis_BlinnPhongToBeckmann)(const double s);
	double				(*Noesis_BeckmannToBlinnPhong)(const double m);

	modelTan4_t			*(*rpgConvertTangentsEx)(int numVerts,
											void *nrmData, rpgeoDataType_e nrmDataType, int nrmStride,
											void *tanData, rpgeoDataType_e tanDataType, int tanStride,
											void *bitanData, rpgeoDataType_e bitanDataType, int bitanStride,
											modelTan4_t *tangents, const int flags, const void *pResv);

	int					(*Noesis_KF_DataCountForRotationType)(const noeKeyFrameRotation_e t);
	int					(*Noesis_KF_DataCountForTranslationType)(const noeKeyFrameTranslation_e t);
	int					(*Noesis_KF_DataCountForScaleType)(const noeKeyFrameScale_e t);
	//that's right, i'm using erp
	int					(*Noesis_KF_ExtraDataCountForRotationAndErpType)(const noeKeyFrameRotation_e t, const noeKeyFrameInterpolation_e lt);
	int					(*Noesis_KF_ExtraDataCountForTranslationAndErpType)(const noeKeyFrameTranslation_e t, const noeKeyFrameInterpolation_e lt);
	int					(*Noesis_KF_ExtraDataCountForScaleAndErpType)(const noeKeyFrameScale_e t, const noeKeyFrameInterpolation_e lt);

	//resize with point filtering
	//flag bit 1 uses texel center instead of corner
	void				(*Noesis_ResampleImageNearest)(unsigned char *src, int srcW, int srcH, unsigned char *dst, int dstW, int dstH, int flags);
	...
	bool				(*Image_DecodeASTC)(unsigned char *pDestBuffer, const unsigned char *pSourceBuffer, const unsigned int sourceBufferSize, int *pBlockDimensions, int *pImageSizes);
	//source is rgba32 * width * height * depth
	bool				(*Image_EncodeASTC)(unsigned char *pDestBuffer, const unsigned char *pSourceBuffer, const unsigned int destBufferSize, const unsigned int sourceBufferSize, int *pBlockDimensions, int *pImageSizes, const int encoderQuality);
	//dest is rgba32 * width * height, pFmt can be: "R", "RG", "Rs", "RGs", "RGB", "RGB1", "sRGB", "RGBA", "sRGBA", "RGBA1", "sRGBA1"
	bool				(*Image_DecodeETC)(void *pDst, const void *pSrc, const int srcSize, const int width, const int height, const char *pFmt);
	...
	bool				(*Image_EncodeETC)(void *pDst, const void *pSrc, const int dstSize, const int width, const int height, const int encFormat, const int encQuality);
	...
	bool				(*Noesis_LastCheckedPartialChecksum)(unsigned int &checksum, const TNoeSize offset, const TNoeSize size);
	...
	void				(*Image_UntilePICA200Raw)(unsigned char *pDst, const unsigned char *pSrc, const int width, const int height, const int bytesPerPixel, const int flags, TResvInt resv);
	void				(*Image_TilePICA200Raw)(unsigned char *pDst, const unsigned char *pSrc, const int width, const int height, const int bytesPerPixel, const int flags, TResvInt resv);
	...
	void				(*Image_UntilePICA200ETC)(unsigned char *pDst, const unsigned char *pSrc, const int width, const int height, const int bytesPerBlock, const int flags, TResvInt resv);
	void				(*Image_TilePICA200ETC)(unsigned char *pDst, const unsigned char *pSrc, const int width, const int height, const int bytesPerBlock, const int flags, TResvInt resv);
	//performs Image_UntilePICA200ETC with flags & 1, deinterleaves alpha, and performs Image_DecodeETC
	void				(*Image_DecodePICA200ETC)(unsigned char *pDst, const unsigned char *pSrc, const int width, const int height, const bool hasAlpha, const int flags, TResvInt resv);

	unsigned int		(*Noesis_CalculateCRC32)(const unsigned char *pData, const TNoeSize dataSize);
	...
	void				(*Image_UntileAMDR600)(unsigned char *pDst, int *pPitchOut, int *pSliceSizeOut, const unsigned char *pSrc, const int width, const int height, const int depth, const int mipIndex, const int format, const int type, const int swizzle, const int tileMode);
	void				(*Image_TileAMDR600)(unsigned char *pDst, int *pPitchOut, int *pSliceSizeOut, const unsigned char *pSrc, const int width, const int height, const int depth, const int mipIndex, const int format, const int type, const int swizzle, const int tileMode);

	//more stuff that was already exposed through extensions
	void				(*Image_UntileBlockLinearGOBs)(unsigned char *pDest, const unsigned int destSize, const unsigned char *pSrc, const unsigned int srcSize, const unsigned int w, const unsigned int h, const unsigned int blockHeight, const int bytesPerElem);
	void				(*Image_TileBlockLinearGOBs)(unsigned char *pDest, const unsigned int destSize, const unsigned char *pSrc, const unsigned int srcSize, const unsigned int w, const unsigned int h, const unsigned int blockHeight, const int bytesPerElem);
	...
	unsigned int		(*Image_TileCalculateBlockLinearGOBBlockHeight)(const unsigned int height, const unsigned int maxBlockHeightL2);
	unsigned int		(*Image_BlockLinearGOBMaxBlockHeightForDefaultLayout)(const unsigned int height, const unsigned int minHeightL2);
	...
	bool				(*Noesis_DeferToOtherHandler)(const char *pOtherName, unsigned char *pNewData, const TNoeSize newDataSize, void *pResv);

	//as above with Image_UntileAMDR600 and Image_TileAMDR600, these routines use AMD's AddrLib
	void				(*Image_UntileAMDRDNA)(unsigned char *pDest, const unsigned __int64 destSize, const unsigned char *pSrc, const unsigned __int64 srcSize, const unsigned __int64 width, const unsigned __int64 height, const unsigned __int64 blockWidth, const unsigned __int64 blockHeight, const unsigned __int64 tileMode, const unsigned __int64 bytesPerElem);
	void				(*Image_TileAMDRDNA)(unsigned char *pDest, const unsigned __int64 destSize, const unsigned char *pSrc, const unsigned __int64 srcSize, const unsigned __int64 width, const unsigned __int64 height, const unsigned __int64 blockWidth, const unsigned __int64 blockHeight, const unsigned __int64 tileMode, const unsigned __int64 bytesPerElem);
	void				(*Image_CalculateAMDRDNAMipInfo)(unsigned __int64 *pMipOffset, unsigned __int64 *pWidthInElements, unsigned __int64 *pHeightInElements, unsigned __int64 *pBlockWidth, unsigned __int64 *pBlockHeight, unsigned __int64 *pBytesPerElem, const unsigned __int64 tileMode, const unsigned __int64 dataFormat, const unsigned __int64 width, const unsigned __int64 height, const unsigned __int64 mipIndex, const unsigned __int64 mipCount);
	...
	int					(*Noesis_GetInternalViewDataSize)();
	//dstSize must be >= Noesis_GetInternalViewDataSize(). returns true on success.
	bool				(*Noesis_GetInternalViewData)(void *pDst, const int dstSize);
	//returns true on success.
	bool				(*Noesis_SetInternalViewData)(const void *pData);
	...
	void				(*Noesis_WriteAndReadbackPS2Explicit32)(void *pData, const int writeWidth, const int writeHeight, const int width, const int height, int bpp);

	//the returned data pointer does not need to be freed, and is subject to processing from -paltransindex/-paltranscolor
	void				*(*Image_InstancedCachedRgb24ToRgba32)(const void *pRgb, const int colorCount);
	void				(*Image_ClearInstancedCache)();

	void				(*Noesis_ResampleImageBicubic)(unsigned char *src, int srcW, int srcH, unsigned char *dst, int dstW, int dstH, const float sharpness);

	void				(*Noesis_ForcePreviewAnimUI)();

	bool				(*Noesis_TwiddlePSP)(BYTE *pixData, int pixDataSize, int width, int height, int bitsPerPixel);

	int					(*Decomp_SegaChannel_SA)(unsigned char *pDst, const unsigned char *pSrc, const unsigned int dstSize, const unsigned int srcSize);
	int					(*Compress_SegaChannel_SA)(unsigned char *pDst, const unsigned char *pSrc, const unsigned int dstSize, const unsigned int srcSize);
...
enum ENoeSetRAPIContextType
{
	kNoeSetRAPIContextType_None = 0,
	kNoeSetRAPIContextType_Preview
};

template<ENoeSetRAPIContextType setContextType>
class CNoePreserveRAPIContext
{
public:
	CNoePreserveRAPIContext()
	{
		g_nfn->NPAPI_PushRAPIContext(NULL);
		UpdateGlobalMemoryAndGetRAPI();
	}

	~CNoePreserveRAPIContext()
	{
		g_nfn->NPAPI_PopRAPIContext(NULL);
	}

	NoeInline noeRAPI_t *UpdateGlobalMemoryAndGetRAPI()
	{
		noeRAPI_t *pRapi = (setContextType == kNoeSetRAPIContextType_Preview) ? g_nfn->NPAPI_GetPreviewRAPI() : NULL;
		if (pRapi)
		{
			g_nfn->NPAPI_SetGlobalMemForRAPI(pRapi);
		}
		return pRapi;
	}
};

typedef CNoePreserveRAPIContext<kNoeSetRAPIContextType_None> TNoePreserveRAPIContext;
typedef CNoePreserveRAPIContext<kNoeSetRAPIContextType_Preview> TNoePreviewRAPIContext;

class CNoePreserveViewContext
{
public:
	CNoePreserveViewContext()
		: mpData(NULL)
		, mPrvCtx()
	{
		noeRAPI_t *pRapi = mPrvCtx.UpdateGlobalMemoryAndGetRAPI();
		if (pRapi)
		{
			const int dataSize = pRapi->Noesis_GetInternalViewDataSize();
			mpData = g_nfn->NPAPI_EngineAlloc(dataSize);
			const bool success = pRapi->Noesis_GetInternalViewData(mpData, dataSize);
			if (!success)
			{
				//silently fail, assuming that we don't have anything loaded
				g_nfn->NPAPI_EngineFree(mpData);
				mpData = NULL;
			}
		}
	}
	...
	~CNoePreserveViewContext()
	{
		if (mpData)
		{
			...
			if (pRapi)
			{
				pRapi->Noesis_SetInternalViewData(mpData);
			}
			g_nfn->NPAPI_EngineFree(mpData);
		}
	}
...
private:
	CNoePreserveViewContext(const CNoePreserveViewContext &other); //implementation intentionally absent

	void *mpData;
	TNoePreviewRAPIContext mPrvCtx;
};
